...
class ClienteBO
{
  ...
  /**
   * Metodo para editar informacion del cliente
   * @param array $datos
   * @param $status
   * @return array $update
   */
  public static function editarPerfil($datos)
  {
    ...
    if ($datos['tipo'] == Constantes::EDITAR_FOTO) {
      $update['foto'] = $datos['foto'];
    }elseif ($datos['tipo'] == Constantes::EDITAR_PASSWORD) {
      $update['password'] =Utilerias::generarPassword($datos['password']);
    }
